...
const x402Steps: DemoStep[] = [
  ...
  },
  {
    ...
    description:
      "API responds with x402 payment requirements — amount, token, recipient",
  ...
  },
  {
    ...
    description:
      "Generates zero-knowledge proof — proves compliance without revealing identity",
  ...
  },
  {
    ...
    description:
      "On-chain compliance stamp via ComplianceRegistry smart contract",
  ...
  },
  {
    ...
    description:
      "Agent retries with payment receipt — premium data delivered successfully",
    poweredBy: "x402 Protocol",
    icon: CheckCircle2,
    status: "idle",
    duration: 800,
...
const affiliateSteps: DemoStep[] = [
  ...
  },
  {
    ...
    description:
      "Shielded transfer to Affiliate — amount hidden on public ledger",
  ...
  },
  {
    ...
    description:
      "Shielded transfer to Merchant — amount hidden on public ledger",
    poweredBy: "Unlink Privacy Pool → Monad",
    icon: Lock,
    status: "idle",
    duration: 1200,
  },
  {
    ...
    description:
      "All parties paid correctly. ZK proof on-chain. No data exposed.",
  ...
  const runFallbackDemo = async () => {
    ...
    for (let i = 0; i < currentSteps.length; i++) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, currentSteps[i].duration),
      );
      if (abortRef.current) return;
      simulatedTimings.push({
        step: currentSteps[i].id,
        durationMs: Date.now() - stepStart,
      });
      setStepTimings([...simulatedTimings]);

      if (
        currentSteps[i].id === "zk-stamp" ||
        currentSteps[i].id === "zk-split"
      ) {
        const hash = `0xZK${Math.random().toString(16).slice(2, 14)}`;
        setProofHash(hash);
      }
      if (
        currentSteps[i].id === "settlement" ||
        currentSteps[i].id === "merchant-pay"
      ) {
      ...
      try {
        ...
        setSteps((prev: DemoStep[]) => {
          ...
          updated[stepIndex] = {
            ...updated[stepIndex],
            status: uiStatus,
            detail: detailStr || updated[stepIndex].detail,
          };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              cursor: "pointer",
              background: isRunning ? "#ef4444" : "#7C3AED",
              color: "white",
              boxShadow: isRunning
          ...
          >
            {isRunning ? (
              ...
              <>
                ...
                {activeDemo === "x402" ? "Agent" : "Affiliate"} Demo
              </>
            )}
          </button>
        </div>
      ...
      <div
        style={{
          background:
            "linear-gradient(135deg, rgba(124,58,237,0.04), rgba(124,58,237,0.08))",
      ...
      >
        {activeDemo === "x402" ? (
          <div>
            ...
            >
              An enterprise AI agent purchases premium financial data from a
              paywalled API using the <strong>x402 protocol</strong>.
              CompliAgent intercepts the payment, verifies compliance, generates
              a <strong>ZK stamp via Unlink</strong>, and settles on{" "}
              <strong>Monad</strong> in under 1 second. The public ledger
              reveals nothing about agent identity, data purchased, or amount.
            </p>
            <div
              style={{
                display: "flex",
            ...
            >
              {[
                {
                  icon: Shield,
                  label: "Privacy: Unlink SDK",
                  color: "#7C3AED",
                },
                {
                  icon: Zap,
                  label: "Settlement: Monad Testnet",
                  color: "#eab308",
                },
                {
                  icon: ShieldCheck,
                  label: "Compliance: On-Chain Registry",
                  color: "#10b981",
                },
              ].map((tag) => (
                <div
          ...
          <div>
            ...
            >
              Merchant (Entity 1) sells a product. Affiliate (Entity 2) earns{" "}
              <strong>15% commission</strong>. Buyer (Entity 3) pays $100 USDC.
              CompliAgent verifies the commission split using{" "}
              <strong>zero-knowledge verification</strong>: proving x + y = z
              without exposing individual amounts. Each transfer is{" "}
              <strong>shielded via Unlink</strong>.
            </p>
            <div
              style={{
                display: "flex",
            ...
            >
              {[
                {
                  icon: Lock,
                  label: "Shielded Transfers: Unlink",
                  color: "#7C3AED",
                },
                {
                  icon: Zap,
                  label: "On-Chain Stamps: Monad",
                  color: "#eab308",
                },
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding: "16px 20px",
                  borderBottom:
                    idx < steps.length - 1 ? "1px solid #f3f4f6" : "none",
          ...
          <div style={{ padding: "24px 20px" }}>
            ...
            >
              {[
                ...
                {
                  label: "CompliAgent Engine",
                  icon: ShieldCheck,
                  color: "#10b981",
                  activeStep: 2,
                },
                {
                  label: "Unlink Privacy Pool",
                  icon: Shield,
                  color: "#7C3AED",
                  activeStep: 3,
                },
                      ...
                      >
                        <node.icon
                          style={{
                            width: 20,
                            height: 20,
                            color: active ? node.color : "#9ca3af",
                            transition: "all 0.5s",
                          }}
                        />
                      </div>
                      ...
                      >
                        <div
                          style={{
                            ...
                            transition: "all 0.5s",
                            background:
                              currentStep > node.activeStep || demoComplete
                                ? "#7C3AED"
                                : "#e5e7eb",
                          }}
                        />
